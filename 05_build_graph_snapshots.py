...
def build_route_stats_lookup(flights):
    """
    Precompute historical ArrDelay mean and std per route × hour × day-of-week.
    ...
    g2 = (work.groupby(["ORIGIN","DEST","hour"])["ArrDelay"]
              .agg(avg_delay="mean", std_delay="std", n="count")
              .reset_index())
    g2 = g2[g2["n"] >= 5]
    lookup_hour = {
    ...
    # Fallback: route only
    g3 = (work.groupby(["ORIGIN","DEST"])["ArrDelay"]
    ...
    global_mean = float(work["ArrDelay"].mean())
    global_std  = float(work["ArrDelay"].std())
    ...
    for ap1 in airports:
        for ap2 in airports:
            ...
            r = float(corr.loc[ap1,ap2]) \
                if ap1 in corr.index and ap2 in corr.columns else 0.1
            records.append({"edge_type":"network",
                             "src_airport":ap1,"dst_airport":ap2,
                             "correlation":round(r,4),
                             "edge_weight":round(float(np.clip(r,0,1)),4)})
    df = pd.DataFrame(records)
    print(f"  Network edges: {len(df):,} | avg weight: {df['edge_weight'].mean():.3f}")
    return df
...
def preaggregate_airport_labels(flights):
    ...
    arr_df = (flights.groupby(["arr_hour","DEST"])
                     ...
                     .reset_index()
                     .rename(columns={"arr_hour":"hour","DEST":"airport"}))
    return {(r.hour, r.airport): r.avg_arr_delay
            for r in arr_df.itertuples(index=False)}
...
def preindex_flights(flights, snapshot_times):
    ...
    for t, t_ns in zip(snapshot_times, snap_times_ns):
        ...
        # Arrivals in [t_ns, t_end)
        lo2 = np.searchsorted(arr_ns_sorted, t_ns,  side="left")
        hi2 = np.searchsorted(arr_ns_sorted, t_end, side="left")
        arr_rows = arr_sort_order[lo2:hi2]

        # Combine and deduplicate
        if len(dep_rows) == 0 and len(arr_rows) == 0:
            snap_to_fids[t] = np.array([], dtype=np.int64)
        elif len(dep_rows) == 0:
            snap_to_fids[t] = np.unique(arr_rows)
        elif len(arr_rows) == 0:
            snap_to_fids[t] = np.unique(dep_rows)
        else:
            combined = np.concatenate([dep_rows, arr_rows])
            snap_to_fids[t] = np.unique(combined)
...
def preextract_flight_arrays(flights, ap2idx, top_hubs, taxi_baseline,
    ...
    fa = {}
    fa["dep_delay"]   = flights["DepDelay"].fillna(0).values.astype(np.float32)
    ...
    fa["arr_delay"]   = flights["ArrDelay"].fillna(np.nan).values.astype(np.float32)
    fa["arr_valid"]   = flights["ArrDelay"].notna().values
    ...
    # Metadata strings for step 7 (keep as object arrays)
...
def build_flight_features_fast(fids, fa, t_ns, same_snap_inbound):
    ...
    taxi_out    = fa["taxi_out"][fids]
    distance    = fa["distance"][fids]
    carrier_d   = fa["carrier_d"][fids]
    ...
    X[:,0]  = np.clip(dep_delay / MAX_DELAY_MIN, -1, 1)
...
def build_flight_labels_fast(fids, fa, t_ns):
    ...
    arr_valid = fa["arr_valid"][fids]
    arr_delay = fa["arr_delay"][fids]
    dep_dt_ns = fa["dep_dt_ns"][fids]

    h2dep = (dep_dt_ns.astype(np.float64) - t_ns) / 3_600_000_000_000

    valid = arr_valid
    y[valid] = arr_delay[valid]

    # y_mask_{h}h selects flights >= h hours from departure.
    # Used in loss: 6h mask trains on far-out flights, 1h mask trains on all flights.
    # This is correct — each horizon trains on the flights actually at that distance.
    for h in LABEL_HORIZONS_FL:
        masks[h] = valid & (h2dep >= h)
...
def build_flight_edges_fast(fids, fa, rot_grp, rot_arr_data,
    ...
    if rot_grp is not None and len(rot_grp) > 0:
        ...
        if leg2_col is not None:
            ...
            for j, l2 in enumerate(leg2_ids):
                if l2 not in fl2local:
                    continue
...
def build_snapshots(airports, ap2idx, snapshot_times,
    ...
    for t in tqdm(snapshot_times, desc="Building snapshots", unit="snap"):
        ...
        # ── Metadata: flight_id only (strings saved to lookup parquet) ────
        # Saves ~5 GB vs storing origin/dest/tail/airline in every snapshot.
        # Step 7 joins flight_id back to flight_lookup.parquet for display.
        if n_fl > 0:
...
def main():
    ...
    X_ap_static, top_hubs, taxi_baseline = build_static_airport_features(
        airports, ap2idx, flights)

    print("\nBuilding feature lookups ...")
    forecast_lookup = build_weather_forecast_lookup(flights)
    traffic_lookup  = build_traffic_lookup(flights, ap2idx)
    # Compute route stats from TRAINING years only — prevents leakage into val/test
    train_flights   = flights[
        flights["dep_datetime"].dt.year.isin(TRAIN_YEARS)].copy()
    print(f"  Route stats computed from {len(train_flights):,} training flights "
          f"({TRAIN_YEARS})")
    ...
    network_edges = build_network_edges(airports, ap2idx, flights)
    network_edges.to_parquet(
        os.path.join(OUTPUT_DIR,"network_edges.parquet"), index=False)
    print("  ✅ Network edges saved")

    tail_propagation_lookup = build_tail_propagation_lookup(flights)
