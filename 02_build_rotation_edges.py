...
MAX_GAP_MIN       = 180    # 3-hour turnaround window = rotation edge threshold
# ────────────────────────────────────────────────────────────────────────────
...
def build_rotation_edges(df):
    ...
    for tail, grp in tails:
        ...
        for i in range(len(grp) - 1):
            ...
            # Gate check — next departure must be at or after this arrival
            gap_min = (nxt["dep_datetime"] - curr["arr_datetime"]).total_seconds() / 60
            if gap_min < MIN_GAP_MIN or gap_min > MAX_GAP_MIN:
                continue

            # Ideal: DEST of curr == ORIGIN of nxt (true rotation)
            # If not, the plane "teleported" — keep but flag
            ...
            rec = {
                ...
                # Graph endpoints: source = leg1 origin, sink = leg2 dest
                "src_airport"     : curr["ORIGIN"],   # upstream node
                "mid_airport"     : curr["DEST"],     # the shared turnaround airport
                "dst_airport"     : nxt["DEST"],      # downstream node
                ...
                # Delay signals from leg 1 that propagate into leg 2
