...
"C:\Users\user\Desktop\Airline_Graphs_Project\flights_2018_2022.parquet"
# Primary: raw flights file
# Fallback: weather-enriched version (same flights + weather columns)
FLIGHTS_PRIMARY  = os.path.join(BASE_DIR, "flights_2018_2022.parquet")
FLIGHTS_FALLBACK = os.path.join(GRAPH_DATA_DIR, "weather_node_features.parquet")
...
def main():
    ...
    # Load flights — prefer raw file, fall back to weather-enriched
    if os.path.exists(FLIGHTS_PRIMARY):
        print(f"\nLoading raw flights: {FLIGHTS_PRIMARY} ...")
        df = pd.read_parquet(FLIGHTS_PRIMARY)
        print(f"  Loaded {len(df):,} rows, {len(df.columns)} columns")
    elif os.path.exists(FLIGHTS_FALLBACK):
        print(f"\nRaw flights not found, using weather-enriched fallback:")
        print(f"  {FLIGHTS_FALLBACK}")
        df = pd.read_parquet(FLIGHTS_FALLBACK)
        print(f"  Loaded {len(df):,} rows, {len(df.columns)} columns")
    else:
        print(f"\n  ❌ Could not find flights file at either location:")
        print(f"     {FLIGHTS_PRIMARY}")
        print(f"     {FLIGHTS_FALLBACK}")
        return

    # Cross-check: if both files exist, confirm they have same row count
    if os.path.exists(FLIGHTS_PRIMARY) and os.path.exists(FLIGHTS_FALLBACK):
        wx = pd.read_parquet(FLIGHTS_FALLBACK, columns=["DepDelay"])
        check("Raw flights and weather-enriched have same row count",
              len(df) == len(wx),
              f"Raw: {len(df):,} | Weather-enriched: {len(wx):,}")
